...
class FTOPSISService
{
    ...
    public function pure_step_5()
    {
        $result = [];

        $type = session('fahp-type')['type'];
    ...
    public function pure_step_8($selected_package_material_index)
    {
        $result = [];
        $to_rank = [];
        $cp_to_rank = [];
        ...
        if ($selected_package_material_index) {
            foreach ($selected_package_material_index as $value) {
                $this->pure_gl_cgi[] = $tmp_cgi[$value];
            }
        } else {
            $this->pure_gl_cgi = $tmp_cgi;
        }
    ...
    public function pure_ranking($material, $collection)
    {
        ...
        foreach ($keys as $key => $value) {
            $result['res'][$value][] = $this->pure_ranking[$key];
    ...
    public function pure_x_bar()
    {
        // ksort($this->pure_gl_cgi);

        $count = count($this->pure_gl_cgi);

        $value = round(
            array_sum($this->pure_gl_cgi) / $count,
            6
        );

        $this->pure_gl_cgi_total = $value;

        return $value;

        return [
            'value' => $value,
            'values_string' => implode(' + ', $this->pure_gl_cgi),
            'values_count' => $count,
        ];
    }

    public function pure_mse()
    {
        // ksort($this->pure_gl_cgi);
        ...
        foreach ($this->pure_gl_cgi as $value) {
            $values[] = round(
                pow(($value - $this->pure_gl_cgi_total), 2),
                6
            );
        }

        $this->pure_gl_mse = $values;
    ...
    public function pure_mae()
    {
        ...
        foreach ($this->pure_gl_cgi as $value) {
            $values[] = round(
                abs( pow(($value - $this->pure_gl_cgi_total), 2) ),
                6
            );
        }
